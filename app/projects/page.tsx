...
import { getProjectsByYear, getUniqueMonthsAndYears } from "@/projects/data";
...
const YearCard: React.FC<{
  ...
  return (
    <Link href={`/projects/${year}`} className="group">
...
export default function ProjectsPage() {
  ...
  const projects = getProjectsByYear(selectedYear || 2025);
  ...
  const yearProjectCounts = useMemo(() => {
  ...
  const getMonthProjectCounts = useMemo(() => {
    return (year: number) =>
      months.map((month) => ({
        month,
    ...
    <div className="container mx-auto py-12 px-4 max-w-6xl">
      ...
      {!selectedYear && (
        ...
        >
          {yearProjectCounts.map(({ year, totalProjects }) => (
            <YearCard key={year} year={year} totalProjects={totalProjects} />
          ))}
        </div>
      )}
      ...
      {selectedYear && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {getMonthProjectCounts(selectedYear).map(
            ({ month, projectCount }) => (
              <MonthCard
                key={month}
                month={month}
